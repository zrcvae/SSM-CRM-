...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".myDate").datetimepicker({
			...
			autoclose:true,//设置选择日期完成后是否关闭日历
			todayBtn:true,//设置是否显示当前日期按钮
		...
		$("#tBody").on("click","input[type='checkbox']",function () {
			...
			}else {//如果列表中所有checkbox至少有一个没选中，则"全选"按钮也取消
			...
			$.ajax({
				...
				success:function (data) {
					if(data.code=="1"){
						...
						queryActivityByConditionForPage($("#demo_page").bs_pagination('getOption','currentPage'),$("#demo_page").bs_pagination('getOption','rowsPerPage'));
		...
		$("#importActivityBtn").click(function () {
			...
			*/
			...
			$.ajax({
				...
				contentType:false,//设置ajax向后台提交参数之前，是否把参数统一转换urlencoded编码，默认为true
				type:'post',
				dataType:'json',
				success:function (data) {
					if(data.code=="1"){
	...
	function queryActivityByConditionForPage(pageNo,pageSize) {
		//当市场活动主页加载完成，查询所有数据第一条和总条数，默认每一页显示10条
		...
		$.ajax({
			...
			success:function (data) {
				...
				$("#demo_page").bs_pagination({
					...
					showGoToPage:true,//显示是否"跳转到"的部分
